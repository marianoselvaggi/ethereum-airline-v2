...
contract('Airline', accounts => {
    ...
    it('should allow customers to buy a fligh providing its value', async () => {
    ...
    it('airline should get the real balance of the contract',async () => {
        ...
        let newAirlineBalanace = await instance.getAirlineBalance();
        
        assert(newAirlineBalanace.toString(),price.toString()+price2.toString());
    });
    ...
    it('should allow customers to redeem loyalty points', async () => {
        ...
        let intialBalance = await web3.eth.getBalance(accounts[0]);
